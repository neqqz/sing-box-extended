...
func (c *MultiplexClient) newClientLocked() (*Client, error) {
	c.clients = pruneIdleClients(c.clients)
	// ГЛАВНОЕ: никогда не держим больше ОДНОГО клиента одновременно в
	// состоянии "ещё ни разу не подключался" (EverConnected()==false).
	// minStreams/maxConnections регулируют масштабирование пула УЖЕ
	// РАБОЧИХ соединений под нагрузкой — они ничего не знают о том, жив
	// ли сервер вообще. На холодном старте (или после того как единственное
	// соединение сдохло) в очередь на getClient() может прилететь сразу
	// пачка конкурентных Dial() ДО того, как хоть один из них успеет
	// провалиться — предыдущий вариант с backoffUntil реагирует только
	// ПОСТФАКТУМ, после первого провала, и потому не спасал именно первый,
	// самый массовый залп. Эта проверка убирает саму возможность залпа:
	// если в пуле уже есть клиент, который ещё дозванивается (или только
	// готов начать), все конкурентные запросы просто встают за ним же
	// (дальше их сериализует dialMu внутри его h2Session), а не открывают
	// каждый свой параллельный TCP/TLS хендшейк к одному и тому же серверу.
	// Как только он реально подключится (EverConnected()==true), это
	// ограничение снимается и пул честно масштабируется по
	// minStreams/maxConnections как обычно.
	for _, t := range c.clients {
		if !t.EverConnected() {
			return t, nil
		}
